...
このドキュメントでは、AWS Bedrockを使用してKnowledge Baseを作成する手順を説明します。以下の手順では、S3バケット、OpenSearchコレクション、および自動同期機能を持つKnowledge Baseの作成方法を示します。

## 前提条件

- AWS CLIがインストール・設定済みであること
- 適切なAWS権限を持つIAMユーザーまたはロールでログインしていること
- 対象リージョンが決定していること（例：us-east-1）
